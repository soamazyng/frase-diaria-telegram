...
- [x] Novo(s) parâmetro(s) SSM, no formato decidido no ticket 23, alimentam o conjunto de
      destinatários autorizados na composição da aplicação.
- [x] Confirmado — não presumido — que nenhuma mudança de política IAM é necessária, já
      que a permissão existente cobre o prefixo `/frase-diaria/*` inteiro.
- [x] Dois chat_ids de fixture, ambos autorizados, cada um pedindo `/frase`, recebem cada
      um o próprio pedido extra, com identidades distintas.
- [x] `/status` responde a qualquer destinatário autorizado.
- [x] Um chat_id fora do conjunto continua sem resposta e sem pedido criado (equivalente
      ao comportamento do AC18 hoje, estendido para um conjunto de autorizados).
- [x] Conversa de grupo continua recusada, independentemente do chat_id.
- [x] Testes cobrindo aceitação dos dois destinatários configurados e recusa de um
